      ...
      <Link href={`/discord/${data.id}`} shallow>
        <a
          className={`block h-full bg-slate-600 shadow-inner ${
            props.active
              ? "rounded-2xl"
              : "rounded-4xl transition-all duration-300 hover:rounded-2xl"
          }`}></a>
      </Link>
    </ListItem>
  )
      ...
      <li className="">
        <button className="aspect-square w-full" onClick={toggleExpand} type="button">
          <div className=" rounded-2xl py-2 shadow-inner transition-colors hover:bg-slate-700">
            <span className="icon text-4xl text-slate-300">folder</span>
          </div>
        </button>
      </li>
      {servers.map((ser) => (
