...
import { authUid } from '../utils/fn-helpers'

const fn = sql`
  create or replace function get_auth_user_person_id()
  returns bigint as $$
    ${db.select({ id: profile.personId }).from(profile).where(eq(profile.createdBy, authUid))};
  $$ language sql stable security definer set search_path = public;
  alter function get_auth_user_person_id() owner to postgres;

  create or replace function get_auth_user_profile_id()
  returns bigint as $$
    ${db.select({ id: profile.id }).from(profile).where(eq(profile.createdBy, authUid))};
  $$ language sql stable security definer set search_path = public;
